...
def case_unknown_feature_slug_errors():
    ...
    assert has_finding(findings, check="unknown-feature", message="user-mgmtt"), \
        "a --feature slug with no spec was not reported"
    assert has_severity(findings, "ERROR", check="unknown-feature"), \
        "an unknown --feature slug was reported below ERROR, so the gate still passes"
    assert has_finding(findings, check="unknown-feature", message="user-mgmt."), \
...
def case_a_file_cannot_implement_and_cover_itself():
    ...
    findings = get_findings(
        ...
        source="# IMPLEMENTS: USERMGMT:REQ-001\n# COVERS: USERMGMT:REQ-001, USERMGMT:UT-001\n",
        test=None)
    assert has_finding(findings, check="trace-test", message="USERMGMT:REQ-001"), \
        "a source file covering itself still satisfied test coverage"
    assert has_finding(findings, check="tag-role", message="COVERS"), \
...
def case_covers_in_a_source_file_names_where_the_tag_actually_is():
    ...
    findings = get_findings(
        ...
        source="# IMPLEMENTS: USERMGMT:REQ-001\n# COVERS: USERMGMT:REQ-001, USERMGMT:UT-001\n",
        test=None)
    assert has_finding(findings, check="trace-test", message="src/users.py"), \
...
def case_implements_in_a_test_file_does_not_satisfy_code_coverage():
    ...
    assert has_finding(findings, check="trace-code", message="USERMGMT:REQ-001"), \
        "an IMPLEMENTS: header in a test file still satisfied code coverage"
    assert has_finding(findings, check="tag-role", message="IMPLEMENTS"), \
...
def case_colocated_go_test_is_classified_as_a_test():
    ...
    assert not has_finding(findings, check="trace-test"), \
        "a colocated Go _test.go file was not recognised as a test"
    assert not has_finding(findings, check="tag-role"), \
...
def case_tag_in_a_plain_text_file_is_not_a_tag():
    ...
    findings = get_findings(
        ...
        extra_files={"notes.txt": "IMPLEMENTS: USERMGMT:REQ-001\n"
                                  "COVERS: USERMGMT:REQ-001, USERMGMT:UT-001\n"})
    assert has_finding(findings, check="trace-code", message="USERMGMT:REQ-001"), \
...
def case_missing_config_uses_built_in_defaults():
    ...
    findings = get_findings(
        ...
        source="# IMPLEMENTS: USERMGMT:REQ-001\n",
        test="# COVERS: USERMGMT:REQ-001, USERMGMT:UT-001\n")
    assert not has_finding(findings, check="config"), \
...
def case_malformed_config_is_reported_not_ignored():
    ...
    assert has_finding(findings, check="config", message="not valid JSON"), \
...
def case_oversized_file_skip_is_reported():
    ...
    assert has_finding(findings, check="skipped-file", message="src/users.py"), \
...
def case_vague_requirement_term_is_flagged():
    ...
    assert has_finding(findings, check="ears", message="unverifiable term"), \
...
def case_compound_requirement_is_flagged():
    ...
    findings = get_ears_findings(
        ...
        "and SHALL bill them.")
    assert has_finding(findings, check="ears", message="SHALL clauses"), \
...
def case_non_ears_leading_keyword_is_flagged():
    ...
    assert has_finding(findings, check="ears", message="not an EARS keyword"), \
        "a non-EARS opening keyword passed"
    assert has_finding(findings, check="ears", message="use WHEN"), \
...
def case_requirement_without_a_subject_is_flagged():
    ...
    assert has_finding(findings, check="ears", message="no subject before SHALL"), \
...
def case_fenced_example_in_a_spec_is_not_a_definition():
    ...
    findings = get_findings(
        ...
        source="# IMPLEMENTS: USERMGMT:REQ-001\n",
        test="# COVERS: USERMGMT:REQ-001, USERMGMT:UT-001\n")
    assert not has_finding(findings, check="recycled-id"), \
...
def case_prose_mentioning_a_requirement_id_is_not_a_definition():
    ...
    findings = get_findings(
        ...
        source="# IMPLEMENTS: USERMGMT:REQ-001\n",
        test="# COVERS: USERMGMT:REQ-001, USERMGMT:UT-001\n")
    assert not has_finding(findings, check="dup-id"), \
...
def case_prose_mentioning_an_ac_does_not_define_it():
    ...
    assert has_finding(findings, check="ac-citation", message="AC-777"), \
...
def case_test_plan_row_for_a_removed_requirement_errors():
    ...
    assert has_finding(findings, check="plan-stale-row", message="REQ-002"), \
...
def case_renamed_test_plan_heading_still_resolves():
    ...
    findings = get_spec_text_findings(
        ...
        "## Requirements\n\n- `REQ-001` (AC-001): The system SHALL sign up users.\n"
        "\n## Tests\n\n| ID | Req |\n|----|-----|\n| UT-001 | REQ-001 |\n")
    assert not has_finding(findings, check="missing-test-plan"), \
...
def case_ordinary_docs_architecture_is_not_legacy():
    ...
    with tempfile.TemporaryDirectory() as root:
        ...
        write_file(root, os.path.join("docs", "architecture.md"),
...
def get_spec_text_findings(spec_text, source=None, test=None, config=None):
    ...
    try:
        write_file(root, os.path.join(".sdlc", "specs", "user-mgmt", "spec.md"),
                   spec_text)
        if config is not None:
            write_file(root, os.path.join(".sdlc", "config.json"),
                       json.dumps(config))
        write_file(root, os.path.join("src", "users.py"),
...
def get_findings(requirements, source, test, nfr_rows=None,
                 ...
                 extra_files=None, config=None, source_path=None, test_path=None):
    ...
    try:
        ...
        if source is not None:
            write_file(root, source_path or os.path.join("src", "users.py"),
                       source + "def signup(): pass\n")
        if test is not None:
            write_file(root, test_path or os.path.join("tests", "test_users.py"),
